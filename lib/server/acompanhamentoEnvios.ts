...
export async function consultarEnvios(id: string) {
  ...
  const { data, error } = await supabaseServer.from('acompanhamento_envios')
...
export async function enviarAcompanhamento(id: string, empresaId: string, canais: Canal[] = ['email', 'whatsapp']) {
  ...
  await Promise.all(canais.map(async canal => {
    const insercao = await supabaseServer.from('acompanhamento_envios').upsert({
    ...
    const { data: envio, error: erroClaim } = await supabaseServer.from('acompanhamento_envios')
          ...
          if (canal === 'email' ? emailConfigurado : whatsappConfigurado) {
            const resposta = await fetch(canal === 'email' ? 'https://api.resend.com/emails'
              ...
              headers: { 'Content-Type': 'application/json',
                ...
                ...(canal === 'email' ? { 'Idempotency-Key': `acompanhamento-${envio.id}` } : {}) },
    ...
    const salvo = await supabaseServer.from('acompanhamento_envios')
