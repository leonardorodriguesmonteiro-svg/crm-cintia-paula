...
begin
  ...
  if v_orcamento.dados_cliente_completos_em is not null then
    return jsonb_build_object('orcamento_id', v_orcamento.id, 'empresa_id', v_orcamento.empresa_id, 'oportunidade_id', v_orcamento.oportunidade_id, 'numero', v_orcamento.numero, 'status', v_orcamento.formalizacao_status, 'ja_completo', true);
  end if;
  ...
  if v_cliente_id is null then
    ...
    values (v_oportunidade.nome_contato, v_cpf, v_oportunidade.celular, coalesce(v_email, v_oportunidade.email), v_cep, v_endereco, v_numero, v_complemento, v_bairro, v_cidade, v_estado, v_oportunidade.origem, 'Cliente', 'Cadastro completado após o aceite da proposta.') returning id into v_cliente_id;
  else
    update public.clientes set cpf = v_cpf, whatsapp = coalesce(nullif(whatsapp, ''), v_oportunidade.celular), email = coalesce(v_email, email, v_oportunidade.email), cep = v_cep, endereco = v_endereco, numero = v_numero, complemento = v_complemento, bairro = v_bairro, cidade = v_cidade, estado = v_estado, updated_at = now() where id = v_cliente_id;
  end if;
...
$$;
revoke all on function public.completar_dados_cliente_proposta_v2_servidor(uuid, text, text, text, text, text, text, text, text, text) from public, anon, authenticated, service_role;
