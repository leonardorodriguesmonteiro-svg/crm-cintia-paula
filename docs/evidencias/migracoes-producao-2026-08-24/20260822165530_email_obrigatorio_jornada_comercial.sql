...
begin
  if v_cpf !~ '^[0-9]{11}$' then raise exception using errcode='22023', message='CPF invalido.'; end if;
  ...
  if v_cliente_id is null then select cliente.id into v_cliente_id from public.clientes cliente where regexp_replace(coalesce(cliente.whatsapp,''),'[^0-9]','','g')=regexp_replace(coalesce(v_oportunidade.celular,''),'[^0-9]','','g') order by cliente.created_at limit 1; end if;
