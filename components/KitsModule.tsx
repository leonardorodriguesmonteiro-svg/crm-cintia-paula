...
export function KitsModule(){
  ...
  function campo(label:string, input:React.ReactNode){return <label className="space-y-1"><span className="text-sm font-medium text-slate-700">{label}</span>{input}</label>}
  return <div className="p-4 md:p-8 space-y-6"><div><h1 className="text-2xl font-bold">Kits</h1><p className="text-slate-500">Cadastre temas, valores e disponibilidade dos kits.</p></div>
    <form onSubmit={save} className="card p-4 grid gap-3 md:grid-cols-3">
      {campo('Código do kit *', <input className="input" placeholder="Ex.: KIT-001" value={form.codigo||''} onChange={e=>setForm({...form,codigo:e.target.value})}/>)}
      {campo('Nome do kit *', <input className="input" placeholder="Nome para identificação" value={form.nome||''} onChange={e=>setForm({...form,nome:e.target.value})}/>)}
      ...
      {campo('Valor da locação (R$)', <input className="input" placeholder="0,00" type="number" min="0" step="0.01" value={form.valor??0} onFocus={e=>e.currentTarget.value==='0'&&e.currentTarget.select()} onClick={e=>e.currentTarget.value==='0'&&e.currentTarget.select()} onChange={e=>setForm({...form,valor:e.target.value})}/>)}
