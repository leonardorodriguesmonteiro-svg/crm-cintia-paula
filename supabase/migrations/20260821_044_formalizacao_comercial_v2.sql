...
--
-- Regra central: uma reserva da nova jornada nunca pode virar Confirmada antes
-- de contrato assinado E sinal pago. Até lá, a reserva existe apenas como
-- contêiner provisório, com status Pendente.

-- ---------------------------------------------------------------------------
-- 1. Hold comercial de 24h e diagnóstico de bloqueio da formalização.
-- ---------------------------------------------------------------------------
...
-- Caso a migration seja aplicada sobre propostas V2 já aceitas, ativa o hold
-- sem alterar propostas históricas legadas.
...
-- ---------------------------------------------------------------------------
-- 2. Disponibilidade passa a considerar reservas confirmadas + holds ativos.
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_consumo_confirmado numeric := 0;
  v_consumo_hold numeric := 0;
begin
  ...
  join public.orcamento_itens item
    on item.orcamento_id = orcamento.id
   and item.kit_id = p_kit_id
  where orcamento.status = 'ACEITA'
    ...
    and not exists (
      select 1
      from public.reservas reserva_confirmada
    ...
    and (
      ...
      + coalesce((
          ...
          join public.reserva_itens item_reserva
            on item_reserva.reserva_id = reserva.id
           and item_reserva.kit_id is not null
          join public.kit_composicao alocado
            on alocado.kit_id = item_reserva.kit_id
      ...
      + coalesce((
          ...
          join public.orcamento_itens item_hold
            on item_hold.orcamento_id = orcamento_hold.id
           and item_hold.kit_id is not null
          join public.kit_composicao alocado
            on alocado.kit_id = item_hold.kit_id
          where alocado.item_id = solicitado.item_id
...
-- O trigger da própria reserva precisa ignorar o hold do orçamento que está
-- sendo convertido, senão o hold bloquearia sua própria confirmação.
...
-- ---------------------------------------------------------------------------
-- 3. Trava de banco contra confirmação prematura da nova jornada.
-- ---------------------------------------------------------------------------
    ...
    if found and (
      ...
      then
        raise exception using
          errcode = '23514',
          message = 'A reserva só pode ser confirmada após contrato assinado e sinal pago.';
      end if;
    end if;
  end if;
...
-- ---------------------------------------------------------------------------
-- 4. Finalizador atômico: assinatura + pagamento + disponibilidade.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Formalização cria RESERVA PROVISÓRIA, nunca Confirmada.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_orcamento.status not in ('ACEITA', 'Aprovado') then
    raise exception 'O cliente precisa aceitar a proposta antes da formalização.';
  end if;

  if v_orcamento.status = 'ACEITA'
    and (
      v_orcamento.dados_cliente_completos_em is null
      or v_orcamento.cliente_id is null
    )
  then
  ...
  if v_orcamento.cliente_id is null then
    raise exception 'Não foi possível identificar o cliente desta proposta.';
  end if;

  if v_orcamento.data_evento is null then
    raise exception 'Informe a data do evento antes de formalizar a proposta.';
  end if;
  ...
  if p_valor_sinal > v_orcamento.total then
    raise exception 'O sinal não pode ser maior que o total da proposta.';
  end if;
  ...
  if v_total_kits = 0 then
    raise exception 'Adicione ao menos um kit à proposta antes da formalização.';
  end if;
  ...
  -- Se o hold venceu, a disponibilidade já foi revalidada acima; renovamos a
  -- janela para o cliente concluir contrato e pagamento.
  if v_orcamento.status = 'ACEITA'
    and coalesce(v_orcamento.hold_expira_em, '-infinity'::timestamptz) <= now()
  then
    update public.orcamentos
    set hold_expira_em = now() + interval '24 hours',
        formalizacao_bloqueio = null
  ...
  if v_reserva_id is null then
    insert into public.reservas (
      ...
      concat_ws(
        E'\n\n',
        'Reserva provisória gerada pela proposta ORC-' || lpad(v_orcamento.numero::text, 4, '0') || '.',
        'A confirmação definitiva depende da assinatura do contrato e do pagamento do sinal.',
  ...
  if v_contrato_id is null then
    ...
    insert into public.contratos (
      ...
      v_numero_contrato,
      'Gerado',
      'Contrato gerado automaticamente na formalização da proposta ORC-' ||
        lpad(v_orcamento.numero::text, 4, '0') || '.'
    ...
    insert into public.contrato_versoes (
      ...
      v_contrato_id,
      1,
      'Contrato gerado automaticamente a partir da proposta ORC-' ||
        lpad(v_orcamento.numero::text, 4, '0') || '.',
      'Rascunho'
    );
  end if;
  ...
  if v_lancamento_id is null then
    insert into public.lancamentos_financeiros (
      ...
      v_reserva_id,
      'Receita',
      'Sinal da proposta ORC-' || lpad(v_orcamento.numero::text, 4, '0'),
      'Sinal',
      p_valor_sinal,
      p_vencimento,
      'Pendente',
      'Cobrança criada na formalização comercial V2.'
  ...
  select case
    when contrato.status = 'Assinado' then
      case when lancamento.status = 'Pago' then 'PRONTA_PARA_CONFIRMAR' else 'AGUARDANDO_PAGAMENTO' end
    when contrato.email_enviado_em is not null or contrato.status = 'Enviado' then 'AGUARDANDO_ASSINATURA'
    else 'CONTRATO_GERADO'
  end
  into v_status_formalizacao
  from public.contratos contrato
  ...
  return jsonb_build_object(
    ...
    'mensagem', 'Reserva provisória, contrato e cobrança do sinal preparados. A reserva será confirmada somente após assinatura e pagamento.'
  );
end;
$$;

-- ---------------------------------------------------------------------------
-- 6. Assinatura e pagamento manuais também obedecem o finalizador central.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if not found or v_orcamento.contrato_id is null then
    raise exception 'Formalize a proposta antes de confirmar a assinatura.';
  end if;

  update public.contratos
  set status = 'Assinado',
      assinado_em = coalesce(assinado_em, v_agora),
      observacoes = concat_ws(
        E'\n',
        observacoes,
        'Assinatura confirmada manualmente pelo ERP em ' ||
  ...
  set status = 'Assinado'
  where contrato_id = v_orcamento.contrato_id;

  update public.orcamentos
  set contrato_assinado_em = coalesce(contrato_assinado_em, v_agora)
  where id = p_orcamento_id;

  v_resultado := public.tentar_confirmar_reserva_formalizada(p_orcamento_id);

  return jsonb_build_object(
    'status', v_resultado->>'status',
    'confirmada', coalesce((v_resultado->>'confirmada')::boolean, false),
    'mensagem', case
      when coalesce((v_resultado->>'confirmada')::boolean, false)
        then 'Contrato assinado, sinal pago e reserva confirmada.'
      else 'Contrato assinado. Aguardando o pagamento do sinal para confirmar a reserva.'
    end
  ) || v_resultado;
end;
$$;
...
begin
  ...
  if char_length(trim(coalesce(p_forma_pagamento, ''))) < 2 then
  ...
  if not found or v_orcamento.lancamento_sinal_id is null or v_orcamento.reserva_id is null then
    raise exception 'Formalize a proposta antes de confirmar o sinal.';
  end if;
  ...
  update public.lancamentos_financeiros
  set status = 'Pago',
      data_pagamento = (v_agora at time zone 'America/Sao_Paulo')::date,
  ...
  insert into public.recebimentos (
    ...
    v_lancamento.id,
    v_lancamento.valor,
    (v_agora at time zone 'America/Sao_Paulo')::date,
    trim(p_forma_pagamento),
    'Pago',
    'Sinal registrado pela formalização comercial V2.'
  )
  ...
  set valor_sinal = v_lancamento.valor,
      ...
      data_pagamento_sinal = (v_agora at time zone 'America/Sao_Paulo')::date,
      forma_pagamento_sinal = trim(p_forma_pagamento)
  where id = v_orcamento.reserva_id;

  update public.orcamentos
  set sinal_pago_em = coalesce(sinal_pago_em, v_agora)
  where id = p_orcamento_id;
  ...
  return jsonb_build_object(
    ...
    'mensagem', case
      when coalesce((v_resultado->>'confirmada')::boolean, false)
        then 'Sinal recebido, contrato assinado e reserva confirmada.'
      else 'Sinal recebido. Aguardando a assinatura do contrato para confirmar a reserva.'
    end
  ) || v_resultado;
end;
$$;
...
-- ---------------------------------------------------------------------------
-- 7. Assinatura pública: assinatura isolada nunca confirma a reserva.
-- ---------------------------------------------------------------------------
...
begin
  if current_setting('request.jwt.claim.role', true) is distinct from 'service_role' then
    raise exception using errcode = '42501', message = 'Função exclusiva do servidor.';
  end if;
  ...
  if char_length(v_documento) not in (11, 14) then
  ...
  if char_length(coalesce(v_documento_cliente, '')) in (11, 14)
    and v_documento_cliente <> v_documento
  then
  ...
  return jsonb_build_object(
    ...
    'mensagem', case
      when coalesce((v_resultado->>'confirmada')::boolean, false)
        then 'Contrato assinado, pagamento identificado e reserva confirmada.'
      else 'Contrato assinado. Conclua o pagamento do sinal para confirmar a reserva.'
    end
  ) || v_resultado;
end;
$$;
...
-- ---------------------------------------------------------------------------
-- 8. Mercado Pago: pagamento isolado nunca confirma a reserva.
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_agora timestamptz := coalesce(p_pago_em, now());
begin
  if current_setting('request.jwt.claim.role', true) is distinct from 'service_role' then
    raise exception using errcode = '42501', message = 'Função exclusiva do servidor.';
  end if;
  ...
  if char_length(trim(coalesce(p_pagamento_id, ''))) < 1 then
    raise exception 'Pagamento do Mercado Pago inválido.';
  end if;

  if v_status_normalizado = 'approved'
    and abs(coalesce(p_valor, 0) - coalesce(v_lancamento.valor, 0)) > 0.01
  then
  ...
  if v_status_normalizado = 'approved' then
    ...
    set status_pagamento = 'Pendente',
        ...
        status_comercial = case
          when status = 'Confirmada' then 'Pagamento pendente'
    ...
    if v_orcamento.id is not null then
      update public.orcamentos
      set sinal_pago_em = null,
          formalizado_em = null,
          formalizacao_status = case
            when contrato_assinado_em is not null then 'AGUARDANDO_PAGAMENTO'
            else 'AGUARDANDO_ASSINATURA'
          end,
          formalizacao_bloqueio = case
            when formalizacao_status = 'RESERVA_CONFIRMADA'
              then 'Pagamento estornado após a confirmação. Revisão comercial necessária.'
            else null
          end
      where id = v_orcamento.id;
    end if;
...
-- ---------------------------------------------------------------------------
-- 9. Wrapper servidor com autorização escopada à empresa da proposta.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 10. Envio/cancelamento do contrato sincroniza a etapa de formalização.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if new.email_enviado_em is not null then
    update public.orcamentos
    set formalizacao_status = 'AGUARDANDO_ASSINATURA'
    where contrato_id = new.id
      and formalizacao_status in (
        'CONTRATO_GERADO',
        'CONTRATO_ENVIADO',
        'AGUARDANDO_ASSINATURA'
      );
    ...
    update public.orcamentos
    set formalizacao_status = 'CONTRATO_GERADO'
    where contrato_id = new.id
      and formalizacao_status in ('CONTRATO_ENVIADO', 'AGUARDANDO_ASSINATURA');
  end if;
...
-- ---------------------------------------------------------------------------
-- 11. Superfície de execução: frontend consulta disponibilidade; operações
-- sensíveis continuam exclusivamente no servidor/service role.
-- ---------------------------------------------------------------------------

revoke all on function public.verificar_disponibilidade_kit_com_hold(uuid, date, date, uuid, uuid)
from public, anon, authenticated;

revoke all on function public.tentar_confirmar_reserva_formalizada(uuid)
from public, anon, authenticated, service_role;

revoke all on function public.formalizar_orcamento_aprovado(uuid, numeric, date)
from public, anon, authenticated, service_role;

revoke all on function public.confirmar_assinatura_formalizacao(uuid)
from public, anon, authenticated, service_role;

revoke all on function public.confirmar_pagamento_sinal_formalizacao(uuid, text)
from public, anon, authenticated, service_role;
...
revoke all on function public.gerenciar_hold_orcamento_v2()
from public, anon, authenticated, service_role;
revoke all on function public.validar_confirmacao_reserva_formalizada()
from public, anon, authenticated, service_role;
revoke all on function public.sincronizar_envio_contrato_formalizacao_v2()
from public, anon, authenticated, service_role;
