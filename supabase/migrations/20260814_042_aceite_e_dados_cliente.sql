...
begin
  ...
  if v_orcamento.oportunidade_id is not null then
    ...
    v_nova_jornada := found and v_oportunidade.etapa in (
      'APROVADA', 'CONVERTIDA_EM_PROPOSTA'
    );
  end if;

  v_nova_jornada := v_nova_jornada
    or v_orcamento.status in ('RASCUNHO', 'ENVIADA', 'ACEITA', 'RECUSADA');
  ...
  if v_orcamento.status in ('Expirado', 'EXPIRADA')
    or (
  ...
  then
    ...
    set status = case when v_nova_jornada then 'EXPIRADA' else 'Expirado' end
    where id = v_orcamento.id;
    raise exception using errcode = '23514', message = 'Esta proposta esta expirada.';
  end if;

  if (v_nova_jornada and v_orcamento.status <> 'ENVIADA')
    or (not v_nova_jornada and v_orcamento.status not in ('Rascunho', 'Enviado'))
  then
    raise exception using errcode = '23514', message = 'Esta proposta nao aceita mais respostas.';
  end if;

  v_status := case
    when v_nova_jornada and p_decisao = 'Aprovado' then 'ACEITA'
    when v_nova_jornada then 'RECUSADA'
    else p_decisao
  end;
  v_resposta := case
    when v_nova_jornada and p_decisao = 'Aprovado' then 'ACEITA'
  ...
  set
    ...
    formalizacao_status = case
      when v_status = 'ACEITA' then 'AGUARDANDO_DADOS'
      when v_status = 'RECUSADA' then 'CANCELADA'
  ...
  if not v_nova_jornada and v_orcamento.oportunidade_id is not null then
    ...
    set
      etapa = case when p_decisao = 'Aprovado' then 'Negociação' else 'Perdido' end,
      motivo_perda = case when p_decisao = 'Recusado'
        then 'Orçamento recusado pelo cliente.' else null end
    where id = v_orcamento.oportunidade_id
...
begin
  ...
  if v_cpf !~ '^[0-9]{11}$'
    or char_length(trim(coalesce(p_endereco, ''))) not between 2 and 300
    or char_length(trim(coalesce(p_bairro, ''))) not between 2 and 120
    or char_length(trim(coalesce(p_cidade, ''))) not between 2 and 120
  then
  ...
  if v_cliente_id is null then
    insert into public.clientes (
      nome, cpf, whatsapp, email, endereco, bairro, cidade, origem, status, observacoes
    ) values (
      v_oportunidade.nome_contato,
      v_cpf,
      v_oportunidade.celular,
      coalesce(v_email, v_oportunidade.email),
      trim(p_endereco),
      trim(p_bairro),
      trim(p_cidade),
    ...
    set
      ...
      email = coalesce(v_email, email, v_oportunidade.email),
...
create or replace function public.enviar_proposta_servidor(
  p_usuario_id uuid,
  p_empresa_id uuid,
  p_orcamento_id uuid
)
...
begin
  ...
  if v_orcamento.status <> 'RASCUNHO' then
    raise exception using errcode = '23514', message = 'Somente uma proposta em rascunho pode ser enviada.';
  end if;
  if v_orcamento.oportunidade_id is null then
    raise exception using errcode = '23514', message = 'Proposta sem pre-reserva vinculada.';
  end if;
  ...
  if not found or v_oportunidade.etapa <> 'APROVADA' then
    raise exception using errcode = '23514', message = 'A pre-reserva precisa estar aprovada.';
  end if;

  perform set_config('app.usuario_id', p_usuario_id::text, true);

  update public.orcamentos
  set status = 'ENVIADA', versao = versao + 1
  where id = v_orcamento.id;

  update public.oportunidades
  set etapa = 'CONVERTIDA_EM_PROPOSTA', versao = versao + 1
  where id = v_oportunidade.id;
...
revoke all on function public.registrar_resposta_publica_orcamento(
  uuid, text, text, text
) from public, anon, authenticated, service_role;

grant execute on function public.registrar_resposta_publica_orcamento(
  uuid, text, text, text
) to service_role;

revoke all on function public.completar_dados_cliente_proposta_servidor(
  uuid, text, text, text, text, text
) from public, anon, authenticated, service_role;

grant execute on function public.completar_dados_cliente_proposta_servidor(
  uuid, text, text, text, text, text
) to service_role;

revoke all on function public.enviar_proposta_servidor(
  uuid, uuid, uuid
) from public, anon, authenticated, service_role;

grant execute on function public.enviar_proposta_servidor(
  uuid, uuid, uuid
) to service_role;

revoke all on function public.sincronizar_status_formalizacao_orcamento()
from public, anon, authenticated, service_role;
